...
def openai_model_profile(model_name: str) -> ModelProfile:
    ...
    # We leave it in here for all models because the `default_structured_output_mode` is `'tool'`, so `structured_text` is only used
    # when the user specifically uses the StructuredTextOutput marker, so an error from the API is acceptable.
